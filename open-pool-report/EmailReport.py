...
class EmailReport(object):
    ...
    def __init__(self, sendto, data, verbose = 0):
        '''
        Constructor
        '''
        self.sendto = sendto
        ...
        self.body = '<p>This is a report summarizing potential workload issues in the OSG' \
                  ...
                  + ' is filtering and summarizing data to help highlight the important' \
                  + ' issues.</p>'
        ...
        for user, details in self.data.items():
            ...
            if ('PeriodicRelease' in details and len(details['PeriodicRelease']) > 0) or \
               ...
               ('OnExitRemove' in details and len(details['OnExitRemove']) > 0):  
                self.body += '<li>{}'.format(self._userlink(user))
                self.body += '<ul>\n'
                
                for category in ['PeriodicRelease', 'PeriodicHold', 'OnExitHold', 'OnExitRemove']:
    ...
    def _userlink(self, user):
        ...
        user = re.sub('@services.ci-connect.net', '', user)
